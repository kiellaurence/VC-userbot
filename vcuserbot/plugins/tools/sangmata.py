...
from ... import *
from ...modules.helpers.events import edit_or_replyy, extract_user
...
async def sg(client: Client, message: Message):
    ...
    lol = await edit_or_replyy(message, "`Processing...`")
    if args:
        try:
            user = await client.get_users(args)
        except Exception:
            return await lol.edit(message, "`Please specify a valid user!`")
    bot = "SangMata_BOT"
    try:
